...
exports.SNOMED_DICTIONARY = void 0;
exports.seed = seed;
const path_1 = __importDefault(require("path"));
const dotenv_1 = __importDefault(require("dotenv"));
dotenv_1.default.config({ path: path_1.default.join(__dirname, '../../../.env') });
const dedup_1 = require("../../../apps/api/src/services/dedup");
const logger_1 = require("../../../apps/api/src/config/logger");
...
async function fetchWithRetry(url, options, retries = 5) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            if (attempt === retries)
                throw err;
            logger_1.logger.warn(`Network request to ${url} failed (attempt ${attempt}/${retries}). Retrying in ${attempt * 2}s... Error: ${err.message}`);
...
async function seed() {
    ...
    try {
        ...
        logger_1.logger.error({ error: err.message }, 'Qdrant Cloud connection failed after retries.');
        return;
    }
    try {
        ...
        if (checkRes.status === 404) {
            logger_1.logger.info(`Creating collection "${COLLECTION_NAME}"...`);
        ...
        let sourceDict = exports.SNOMED_DICTIONARY;
        try {
            const fs = require('fs');
            const jsonPath = path_1.default.join(__dirname, 'snomed-parsed-dictionary.json');
            if (fs.existsSync(jsonPath)) {
                logger_1.logger.info('Loading parsed RF2 dictionary from JSON file...');
                ...
                const existingCodes = new Set(exports.SNOMED_DICTIONARY.map((r) => r.code));
                sourceDict = [
                    ...exports.SNOMED_DICTIONARY,
                    ...parsed.filter((r) => !existingCodes.has(r.code))
                ];
                logger_1.logger.info(`Loaded parsed RF2 dictionary from JSON. Merged total: ${sourceDict.length}`);
            }
            else {
                const rf2Module = require('./snomed-parsed-dictionary-dictionary');
                if (rf2Module && rf2Module.SNOMED_RF2_DICTIONARY) {
                    let parsed = rf2Module.SNOMED_RF2_DICTIONARY;
                    const limit = process.env.SEED_LIMIT ? parseInt(process.env.SEED_LIMIT, 10) : 2000;
                    if (limit > 0 && parsed.length > limit) {
                        parsed = parsed.slice(0, limit);
                    }
                    ...
                    logger_1.logger.info(`Loaded parsed RF2 dictionary from TS. Merged total: ${sourceDict.length}`);
                }
            }
        }
        catch (err) {
            logger_1.logger.warn(`Failed to load parsed RF2 dictionary: ${err.message}. Using default dictionary.`);
        }
        logger_1.logger.info(`Collection "${COLLECTION_NAME}" ready. Seeding ${sourceDict.length} findings in batches...`);
        ...
        for (let i = 0; i < sourceDict.length; i += BATCH_SIZE) {
            ...
            try {
                ...
                const vectors = await (0, dedup_1.generateOllamaBatchEmbeddings)(textsToEmbed);
                ...
                if (upsertRes.ok) {
                    ...
                    logger_1.logger.error(`Upsert batch failed at index ${i}: ${errText}`);
                }
            }
            catch (err) {
                failed += batch.length;
                logger_1.logger.error(`Error processing batch starting at ${i}: ${err.message}`);
            }
            if ((upserted + failed) % 100 < BATCH_SIZE) {
                const elapsed = ((Date.now() - startTime) / 1000).toFixed(0);
                logger_1.logger.info(`Progress: ${upserted}/${sourceDict.length} (${failed} failed) — ${elapsed}s elapsed`);
            }
        }
        logger_1.logger.info(`SNOMED Qdrant seeding completed. ${upserted} upserted, ${failed} failed.`);
    }
    catch (err) {
        logger_1.logger.error({ error: err.message }, 'Failed to seed SNOMED collection');
    }
}
